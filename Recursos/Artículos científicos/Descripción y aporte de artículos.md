...
|---|---|---|---|---|
| IoT-based indoor environmental monitoring system using multi-parameter sensing and ESP32-WROOM integration | Bakare & Abubaker, 2026 | Discover Electronics | Sistema IoT multiparamétrico con ESP32 | Error menor a 2%, correlación mayor a 0.98, latencia de 215 a 310 ms vía MQTT |
| Wearable Devices for Environmental Monitoring in the Built Environment: A Systematic Review | Salamone, Masullo & Sibilio, 2021 | Sensors (MDPI) | Revisión sistemática de wearables ambientales | Validación consolidada del uso de sensores junto con comunicación inalámbrica para salud preventiva |
...
Desarrolla un sistema IoT basado en ESP32 capaz de medir simultáneamente temperatura, humedad, iluminación y calidad del aire. El sistema utiliza el protocolo MQTT para transmitir datos en tiempo real a la nube, logrando baja latencia (aproximadamente 215 a 310 ms). Implementa una arquitectura escalable con procesamiento tanto en la nube como en el propio dispositivo (edge computing), y reporta errores de medición menores al 2% con coeficientes de correlación superiores a 0.98.

### Aporte para EcoAlerta

Este artículo es uno de los respaldos técnicos más sólidos del proyecto, ya que valida directamente el uso del ESP32 como núcleo de sistemas IoT ambientales, que es exactamente la arquitectura que el equipo propone. Demuestra además que es viable integrar múltiples sensores y transmitir datos en tiempo real hacia plataformas digitales con alta precisión y baja latencia, reforzando la idea de convertir datos ambientales en información útil y procesable para el usuario, que es el objetivo central de EcoAlerta al generar recomendaciones personalizadas. También evidencia que estos sistemas pueden ser económicos, energéticamente eficientes y escalables, lo cual respalda la factibilidad técnica y financiera del prototipo.

**Hallazgos aplicables al proyecto:**
- ESP32 validado como núcleo de sistemas IoT ambientales multiparamétricos.
- MQTT como protocolo eficiente para transmisión en tiempo real con baja latencia.
- Arquitectura edge y cloud computing aplicable al diseño del prototipo.
- Alta precisión de medición (error menor a 2%) alcanzable con sensores de bajo costo.
...
| Validación de protocolo de transmisión (MQTT) | Sí | No específico |
| Precisión cuantificada de sensores | Sí (error menor a 2%) | No cuantificado |
...
Juntos, ambos artículos cubren los dos pilares científicos de EcoAlerta. Por un lado está la viabilidad técnica concreta de una arquitectura ESP32 con sensores y transmisión en tiempo real con métricas de precisión cuantificadas, y por otro está el respaldo consolidado a nivel de literatura sobre la efectividad de wearables ambientales para salud preventiva. En conjunto, confirman que la base tecnológica de EcoAlerta está alineada con la evidencia científica disponible y que la brecha que el proyecto busca cubrir, un monitoreo UV personalizado con recomendaciones en tiempo real, es consistente con las tendencias de investigación actuales.
